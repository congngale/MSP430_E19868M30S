...
void sx1276_init(radio_events_t* events) {
  ...
  GPIO_setAsInputPinWithPullDownResistor( GPIO_PORT_P8, GPIO_PIN1 );          // IRQ pin
  GPIO_selectInterruptEdge( GPIO_PORT_P8, GPIO_PIN1, GPIO_LOW_TO_HIGH_TRANSITION );
...
void sx1275_reset() {
    ...
    GPIO_setAsOutputPin( GPIO_PORT_P8, GPIO_PIN2 );                             // Reset
...
void sx1276_send(uint8_t *buffer, uint8_t size) {
	...
	if(sx1276.Settings.LoRaPacketHandler.Size != size){
	  myUart_writeBuf( BACKCHANNEL_UART, "Write buffer to SPI", NULL, CRLF );
...
#ifndef LOWPOWER
      GPIO_toggleOutputOnPin( GPIO_PORT_P6, GPIO_PIN3 );   // FIFO-Write Time
#endif
      // Write payload buffer
...
#ifndef LOWPOWER
  GPIO_setOutputHighOnPin( GPIO_PORT_P7, GPIO_PIN1 );   // Sending Time
#endif
}
...
void Port_1 (void) {
...
#ifndef LOWPOWER
    GPIO_setOutputHighOnPin( GPIO_PORT_P6, GPIO_PIN3 ); // Wake Up Time
    GPIO_setOutputLowOnPin( GPIO_PORT_P7, GPIO_PIN1 );  // Sending Time
#endif

    if(GPIO_getInterruptStatus( GPIO_PORT_P8, GPIO_PIN1 )){
#ifndef SPIARRAY
    ...
    GPIO_clearInterrupt( GPIO_PORT_P8, GPIO_PIN1 );     // IRQ Flag

#ifndef LOWPOWER
    GPIO_setOutputLowOnPin( GPIO_PORT_P6, GPIO_PIN3 );  // Wake Up Time
#endif
